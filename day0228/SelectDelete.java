...
public class SelectDelete extends JFrame {
	SignIn main;
	JPanel jp_stotal;
	...
	SelectDelete(SignIn main,ArrayList<Integer> numList){
		this.main = main;
		this.numList = numList;
	}
	void selectList() {
		jp_stotal =new JPanel();
		jl_updateNum = new JLabel("삭제할 번호 >>>");
			...
			if(obj == selectbtn) {
				...
				if(numList.size() >=num) {
					if(JOptionPane.showConfirmDialog(null, "정말 삭제하시겠습니까?")==0)
						main.voList.remove(numList.get(num-1)-1);
				}else{
